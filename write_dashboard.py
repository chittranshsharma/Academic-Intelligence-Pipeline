...
function handleSSE(ev){
  ...
  else if(ev.type==='info'){addFeed('mode_switch','\ud83e\udd16 '+e(ev.message));}
  else if(ev.type==='mode'){
    const isPlay=ev.engine==='playwright';
    const badge=isPlay?'<span style="background:rgba(124,58,237,.15);color:#a78bfa;border:1px solid rgba(124,58,237,.3);padding:1px 8px;border-radius:10px;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;">PLAYWRIGHT</span>':'<span style="background:rgba(37,99,235,.1);color:#60a5fa;border:1px solid rgba(37,99,235,.3);padding:1px 8px;border-radius:10px;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;">HTTPX</span>';
    addFeed('page','\u26a1 Engine: '+badge+(isPlay?' (Headless Chromium \u2014 JS pages supported)':' (Fast static fetch)'));
  }
  ...
  else if(ev.type==='error'){addFeed('error','\u274c '+e(ev.message));showAlert('scrapeAlert','\u274c <pre style="white-space:pre-wrap;font-size:11px;margin-top:6px;">'+e(ev.message)+'</pre>','error');stopScrapeUI();}
}
